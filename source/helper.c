...
char* get_str(char* key_name) {
	...
	if (!file) {
		printf("The offsets.txt file was not found. Using default offsets.\n");

		file = fopen("offsets.txt", "w");
		if (file) {
			fprintf(file, "m_vOldOrigin=4900\n");
			fprintf(file, "m_iHealth=836\n");
			fprintf(file, "m_hPlayerPawn=2060\n");
			fprintf(file, "m_iTeamNum=995\n");
			fclose(file);
		}
		else {
...
void LoopThroughPlayers(HDC hdcBuffer) {
	...
	RGB esp_box_color_enemy = { 255, 255, 0 };
